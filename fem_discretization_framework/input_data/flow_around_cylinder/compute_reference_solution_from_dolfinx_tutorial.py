...
# ## Adaptation
#
# This code was adapted to suitably generate the reference solution for the problem under consideration. The desired output here are
#
#     - coordinates_for_visualization.npy
...
if mesh_comm.rank == model_rank:
    ...
    gmsh.option.setNumber("Mesh.MeshSizeMax", r/1.8)
...
if mesh_comm.rank == model_rank:
    ...
    gmsh.model.mesh.setOrder(1) # DO NOT SET THIS TO 2, THEN THE facet2vertex and the vertex2facet connectivity breaks
...
plt.plot(mesh.geometry.x[:,0], mesh.geometry.x[:, 1], '.')
...
for i in range(num_steps):
    ...
    u_.x.scatter_forward() ### THIS IS THE TARGET!

    if i % 160 == 0:
        u_eval = u_.eval(points, cell_indices)  
        p_eval = p_.eval(points, cell_indices)  
        
        # write solution to df
        df_tmp = pd.DataFrame({
            "u1_sol_t"+'{0:.1f}'.format(dt*i) : u_eval[:, 0],
            "u2_sol_t"+'{0:.1f}'.format(dt*i) : u_eval[:, 1],
            "p_sol_t"+'{0:.1f}'.format(dt*i) : p_eval[:, 0],
        })
...
from src.mode_computations import evaluate_function
...
glyphs = function_grid.glyph(orient="u", factor=0.06)
